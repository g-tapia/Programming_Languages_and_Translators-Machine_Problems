...
(test-case "test-simplify"
           ;adding by zero cases, and multiplying by zero
           (define exp1 '(+ (* 100 y) 0))
           (check-equal? (simplify exp1) '(* 100 y))
           (define x '(+ 0 (* 30 x)))
           (check-equal? (simplify x) '(* 30 x))
           (define exp2 '(+ (* 0 y) 1))
           (check-equal? (simplify exp2) 1)
           
           ;multiplying by one cases, also multiplication of zero and one
           (define y '(* 1 (+ x 10000000)))
           (check-equal? (simplify y) '(+ x 10000000))
           (define exp3 '(* (* x 3) 1))
           (check-equal? (simplify exp3) '(* x 3))
           (define exp4 '(+ (* 0 y) 1))
           (check-equal? (simplify exp4) 1)
           (define a '(+ (* x x) (* 1 y)))
           (check-equal? (simplify a) '(+ (* x x) (* 1 y)))
           (define b '(+ (* x x) (* 0 y)))
           (check-equal? (simplify b) '(* x x)))

           ;I tried writing a check equal for these but I kept getting an unbound +
           ;error, it was driving me crazy!!! it works for the others but not these
           ;if you can please paste these or other examples, they will work
           
           ;addition/multiplication involving only numeric operands
          ; (simplify '(+ 1 (* 2 3) (* 3 4) (* 20 30)))   ; ---> 619
          ; (simplify '(+ 1 (+ 3 4) (* 20 30)))    ; --> 608
          ; (simplify '(+ 1 (+ 3000000 4) (* 1000000 0)))   ; -->3000005
